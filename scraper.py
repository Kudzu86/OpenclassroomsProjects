...
# Fonction pour création d'un fichier CSV avec écriture des infos extraites
def create_csv_file(csv_filename, all_books_url, category_directory):
    csv_path = os.path.join(category_directory, csv_filename)
...
def get_book_info(product_page_url, image_directory):
    ...
    image_filename = get_img(image_url, title, image_directory)
    book_info = {
...
def get_all_books(base_url):
    ...
    for category in categories:
        ...
        category_books = get_book_links_from_category(category['url'])
        category_directory = os.path.join("Books", category['name'])
        os.makedirs(category_directory, exist_ok=True)
        csv_filename = f"{category['name'].replace(' ', '_')}.csv"

        # Appel à la fonction pour créer les fichiers CSV
